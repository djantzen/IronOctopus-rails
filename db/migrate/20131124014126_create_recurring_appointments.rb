  ...
  def up
    execute <<-EOS
      create table application.recurring_appointments (
        recurring_appointment_id serial primary key,
      ...
      grant select, usage, update on application.recurring_appointments_recurring_appointment_id_seq to writer;
      ...
      create unique index on application.recurring_appointments (trainer_id, day_of_week, time_slot);
    EOS
  end
